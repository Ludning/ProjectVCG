...
namespace CodeBlockSystem
{
    ...
    {
        // 변수
        [SerializeField] private CodeBlockBase[] codeBlocks; // 제공하는 코드 블록
        ...
        private void Awake() // Awake()
        {
            ...
            foreach (CodeBlockBase codeBlock in codeBlocks)
            {
        ...
        // 코드 블록을 생성하는 함수
        private void CreateCodeBlock(CodeBlockBase[] codeBlock, Transform[] blockTransform)
        {
        ...
        // 코드 블록을 선택하는 함수
        private void SelectCodeBlock(CodeBlockBase codeBlock)
        {
            // 코드 블록을 옮기는 것이 아니라, 복사본을 생성하여 실행자에 전달합니다.
            CodeBlockBase selectedCodeBlock = Instantiate(codeBlock);
